...
export class ImportService {
    ...
    async importAlcohol(alcoholTypes: string): Promise<ImportResult> { 
        ...
        try {
            ...
            this.logger.log(`Fetched ${rawData.length} items for type: ${alcoholTypes}`);
            const result = await this.processBatch(rawData, handler);

            const duration = Date.now() - startTime;
            this.logger.log(`Import completed in ${duration}ms for type: ${alcoholTypes} - ${result.stats?.totalProcessed} items processed`);
    ...
    private async processBatch(rawData: any[], handler: any): Promise<ImportResult> {
        ...
        this.logger.debug(`Processing batch ${batchNumber}/${totalBatches} (${batch.length} items)`);
